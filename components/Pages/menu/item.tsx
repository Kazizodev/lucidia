...
const Item = ({ name, description, price, image, rate }: Props) => {
  ...
  return (
    <div className="lg:w-[48%] w-full mb-4 transition-all duration-500">
      <div className={cn("flex", full && "flex-col")}>
        <Image
          src={image}
          draggable="false"
        ...
        <div className="flex flex-col w-full">
          <div className="flex align-end">
            <h3 className="font-bold lg:text-2xl text-xl leading-none">{name}</h3>
            <div className="radial-circle flex flex-1 h-[4px] relative bg-repeat-x mx-2 self-center md:self-end" />
            <p className="font-bold text-lg">{exchange ? formatter("LBP").format(price * rate) : formatter("usd").format(price)}</p>
          </div>

          <p className="mt-1 whitespace-pre-line font-[400] text-muted-foreground">{description}</p>
        </div>
      </div>
    </div>
  )
